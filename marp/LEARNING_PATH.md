# Tutorial Learning Path

A structured mini-course from beginner to AI developer.

**Legend**: 🟢 Beginner | 🟡 Intermediate | 🔴 Advanced

---

## Phase 1: Foundation
### Terminal & Tools
- [ ] 🟢 Mac Terminal
- [ ] 🟢 Windows Terminal
- [ ] 🟢 Install VS Code
- [ ] 🟢 Markdown
- [ ] 🟢 What is a Browser
- [ ] 🟡 DevTools CSS Inspection

### General
- [ ] 🟢 File Structure
- [ ] 🟢 Write a README

---

## Phase 2: Version Control
### Git
- [ ] 🟢 Install Git
- [ ] 🟢 Git vs GitHub
- [ ] 🟡 Undo Mistakes in Git
- [ ] 🟡 Good Commit Messages
- [ ] 🟡 Collaborate on GitHub

---

## Phase 3: Web Development
### Web Dev Basics
- [ ] 🟢 Basic Web Structure
- [ ] 🟡 CSS Flexbox
- [ ] 🟢 How to Pick Color
- [ ] 🟢 Google Fonts
- [ ] 🟢 Domain Names
- [ ] 🟡 Responsive Design
- [ ] 🟡 Image Optimization
- [ ] 🟢 Favicon
- [ ] 🟢 What is an API

---

## Phase 4: Python Fundamentals
### Python Basics
- [ ] 🟢 Mac Install Python
- [ ] 🟢 Windows Install Python
- [ ] 🟡 Virtual Environment
- [ ] 🟢 Python Scripting
- [ ] 🟡 CSV Files
- [ ] 🟡 Python APIs

### Python Intermediate
- [ ] 🟡 Pandas for Excel Users
- [ ] 🔴 Web Scraping
- [ ] 🟡 Send Email with Python
- [ ] 🟡 Schedule Python Scripts
- [ ] 🟡 Python Calculator with Tkinter

---

## Phase 5: AI Prompting
### Prompt Engineering
- [ ] 🟢 Better AI Prompts
- [ ] 🟡 5 Prompt Frameworks
- [ ] 🟢 ChatGPT as Personal Assistant
- [ ] 🟡 Prompting for Coding
- [ ] 🟡 Structured AI Outputs

---

## Phase 6: AI Agents
### Agent Basics
- [ ] 🟢 What is an AI Agent
- [ ] 🟡 Build First AI Agent
- [ ] 🟡 AI Agent Tools
- [ ] 🟡 Single vs Multi Agent
- [ ] 🟡 AI Agent Memory

---

## Phase 7: Agentic Coding
### AI-Assisted Development
- [ ] 🟢 What is Agentic Coding
- [ ] 🟡 Debug with AI
- [ ] 🟡 Build Apps with AI
- [ ] 🔴 Break Down Projects for AI
- [ ] 🟡 Review AI Code

---

## Phase 8: AI Automations
### Practical Automations
- [ ] 🟡 Automate Daily Tasks
- [ ] 🟡 Connect Apps with AI
- [ ] 🟡 AI Workflow
- [ ] 🟡 Spreadsheets to Automations
- [ ] 🟡 Schedule Automations

---

## Bonus: Career & Content
### Career
- [ ] 🟢 Build Portfolio
- [ ] 🟢 LinkedIn for Developers
- [ ] 🟡 Open Source First Contribution
- [ ] 🟢 Learn Faster
- [ ] 🟢 Document Projects
- [ ] 🟢 Choose What to Learn
- [ ] 🟢 Stay Consistent

### Content / Creator
- [ ] 🟢 10 AI Prompts That Save Hours
- [ ] 🟢 Replaced 5 Hours with AI
- [ ] 🟡 5 AI Automations Today

---

## Progress Summary

| Phase | Topic | 🟢 | 🟡 | 🔴 | Total |
|-------|-------|----|----|----|-------|
| 1 | Foundation | 6 | 1 | 0 | 7 |
| 2 | Git | 2 | 3 | 0 | 5 |
| 3 | Web Dev | 5 | 4 | 0 | 9 |
| 4 | Python | 3 | 7 | 1 | 11 |
| 5 | AI Prompting | 2 | 3 | 0 | 5 |
| 6 | AI Agents | 1 | 4 | 0 | 5 |
| 7 | Agentic Coding | 1 | 3 | 1 | 5 |
| 8 | Automations | 0 | 5 | 0 | 5 |
| — | Bonus: Career | 6 | 1 | 0 | 7 |
| — | Bonus: Content | 2 | 1 | 0 | 3 |
| **Main Course Total** | | **20** | **30** | **2** | **52** |

---

## Quick-Start Path (Bare Minimum)
For total beginners who want the fastest path to building something:

1. 🟢 Mac Terminal / Windows Terminal
2. 🟢 Install VS Code
3. 🟢 Install Git
4. 🟢 Git vs GitHub
5. 🟢 Basic Web Structure
6. 🟢 Python Scripting

**Total**: 6 tutorials → ~3 hours

---

## Created
- 18 existing tutorials
- 34 new placeholder tutorials
- 52 main course tutorials (+ 10 bonus)